...
include '../config.php';
$sql = "SELECT*FROM tbperjalanan WHERE idCatatan='$idcatatan'";
...
if ( isset( $_POST['ubah'] ) ) {
    ...
    include '../config.php';
    $sql = "UPDATE tbperjalanan SET tanggal='$tanggal', waktu='$waktu', lokasi='$lokasi', suhu='$suhu' WHERE idCatatan='$idcatatan'";
    $query = mysqli_query( $conn, $sql );

    if ( $query ) {
        echo '<script>alert("Data catatan berhasil diubah!");</script>';
        echo '<script>window.location.assign("hafizh_catatan.php");</script>';
    } else {
